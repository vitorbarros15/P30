...
describe('CandidatesService', () => {
    ...
    beforeEach(async () => {
        ...
        candidateModel = module.get((0, mongoose_1.getModelToken)(candidate_schema_1.Candidate.name));
    });
    afterEach(() => {
        jest.clearAllMocks();
    });
    describe('create', () => {
        ...
        it('should throw ConflictException when email already exists', async () => {
            ...
            await expect(service.create(createCandidateDto)).rejects.toThrow(business_exception_1.ConflictException);
            expect(candidateModel.findOne).toHaveBeenCalledWith({
                email: createCandidateDto.email,
            });
        });
    ...
    describe('findOne', () => {
        ...
        it('should throw NotFoundException when candidate not found', async () => {
            ...
            await expect(service.findOne(candidateId)).rejects.toThrow(business_exception_1.NotFoundException);
            expect(candidateModel.findById).toHaveBeenCalledWith(candidateId);
        });
    });
    describe('update', () => {
        it('should update a candidate successfully', async () => {
            ...
            expect(candidateModel.findByIdAndUpdate).toHaveBeenCalledWith(candidateId, { ...updateCandidateDto, skills: ['javascript', 'node.js', 'react', 'typescript'] }, { new: true, runValidators: true });
        ...
        it('should throw ValidationException for invalid id', async () => {
            ...
            await expect(service.update(invalidId, updateCandidateDto)).rejects.toThrow(business_exception_1.ValidationException);
        });
        it('should throw NotFoundException when candidate not found', async () => {
            const candidateId = '507f1f77bcf86cd799439011';
            const updateCandidateDto = {
            ...
            await expect(service.update(candidateId, updateCandidateDto)).rejects.toThrow(business_exception_1.NotFoundException);
            expect(candidateModel.findByIdAndUpdate).toHaveBeenCalledWith(candidateId, updateCandidateDto, { new: true, runValidators: true });
        });
    });
    describe('remove', () => {
        ...
        it('should throw ValidationException for invalid id', async () => {
            const invalidId = 'invalid-id';
            await expect(service.remove(invalidId)).rejects.toThrow(business_exception_1.ValidationException);
        });
        it('should throw NotFoundException when candidate not found', async () => {
            const candidateId = '507f1f77bcf86cd799439011';
            candidateModel.findByIdAndDelete.mockResolvedValue(null);
            await expect(service.remove(candidateId)).rejects.toThrow(business_exception_1.NotFoundException);
            expect(candidateModel.findByIdAndDelete).toHaveBeenCalledWith(candidateId);
        });
    });
    describe('inviteCandidate', () => {
        it('should invite a candidate successfully', async () => {
            ...
            expect(candidateModel.findByIdAndUpdate).toHaveBeenCalledWith(candidateId, { isInvited: true }, { new: true });
            expect(result).toEqual(invitedCandidate);
        });
        it('should throw ValidationException for invalid id', async () => {
            const invalidId = 'invalid-id';
            await expect(service.inviteCandidate(invalidId)).rejects.toThrow(business_exception_1.ValidationException);
        });
        it('should throw NotFoundException when candidate not found', async () => {
            const candidateId = '507f1f77bcf86cd799439011';
            candidateModel.findByIdAndUpdate.mockResolvedValue(null);
            await expect(service.inviteCandidate(candidateId)).rejects.toThrow(business_exception_1.NotFoundException);
            expect(candidateModel.findByIdAndUpdate).toHaveBeenCalledWith(candidateId, { isInvited: true }, { new: true });
        });
    });
    describe('uninviteCandidate', () => {
        it('should uninvite a candidate successfully', async () => {
            ...
            expect(candidateModel.findByIdAndUpdate).toHaveBeenCalledWith(candidateId, { isInvited: false }, { new: true });
            expect(result).toEqual(uninvitedCandidate);
        });
        it('should throw ValidationException for invalid id', async () => {
            const invalidId = 'invalid-id';
            await expect(service.uninviteCandidate(invalidId)).rejects.toThrow(business_exception_1.ValidationException);
        });
        it('should throw NotFoundException when candidate not found', async () => {
            const candidateId = '507f1f77bcf86cd799439011';
            candidateModel.findByIdAndUpdate.mockResolvedValue(null);
            await expect(service.uninviteCandidate(candidateId)).rejects.toThrow(business_exception_1.NotFoundException);
            expect(candidateModel.findByIdAndUpdate).toHaveBeenCalledWith(candidateId, { isInvited: false }, { new: true });
        });
    });
});
